import type {QueryArrayResult, QueryConfig, QueryConfigValues, QueryResult, QueryResultRow} from 'pg'
import pgD from 'pg'
const { Pool } = pgD
...
interface ConnectResult<R extends QueryResultRow = any, I extends any[] = any[]> {
  query: (queryTextOrConfig: string, values?: I, callback?: (err: Error, result: QueryResult<R>) => void) => Promise<QueryResult<R> | undefined>
  drain: () => Promise<void>
}
export function Connect(dbUrl:string): ConnectResult {
  ...
  function query<R extends QueryResultRow = any, I extends any[] = any[]>(queryTextOrConfig: string , values?: QueryConfigValues<I>[], callback?: (err: Error, result: QueryResult<R>) => void): Promise<QueryResult<R>|QueryArrayResult<any[]> | undefined> {
    return pool
      .connect()
      .then((client) => {
...
export const pg = Connect(process.env.DATABASE_URL || '')
